        ...
        switch(match){
            case MOVIE:
                numDeleted = db.delete(
                        MoviesContract.MovieEntry.TABLE_FAVORITES, selection, selectionArgs);
                // reset _ID
                db.execSQL("DELETE FROM SQLITE_SEQUENCE WHERE NAME = '" +
                        MoviesContract.MovieEntry.TABLE_FAVORITES + "'");
                break;
            case MOVIE_WITH_ID:
                numDeleted = db.delete(MoviesContract.MovieEntry.TABLE_FAVORITES,
                        MoviesContract.MovieEntry._ID + " = ?",
                        new String[]{String.valueOf(ContentUris.parseId(uri))});
                // reset _ID
                db.execSQL("DELETE FROM SQLITE_SEQUENCE WHERE NAME = '" +
            ...
            case MOVIE:
                ...
                try{
                    for(ContentValues value : values){
                        ...
                        long _id = -1;
                        try{
                            _id = db.insertOrThrow(MoviesContract.MovieEntry.TABLE_FAVORITES,
                                    null, value);
